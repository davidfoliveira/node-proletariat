 ...
 * constructor:
 *
 *   new MessageStream(format[,socket])
 *
 * methods:
 *
 *   - reveiceChunk(chunk)
...
function MessageStream(con,opts) {
	...
	if ( !opts.format || typeof opts.format != "string" )
		opts.format = "string";
	if ( !opts.MSG_MAXSIZE || typeof opts.MSG_MAXSIZE != "number" )
		opts.MSG_MAXSIZE = MSG_MAXSIZE;

	// Import options
	self.format = opts.format;
	self.MSG_MAXSIZE = opts.MSG_MAXSIZE;
	self.con = con;
	...
	// Methods
	self.receiveChunk = msgStreamReceiveChunk;
	self.sendMessage = sendMessage;

	// If has a connection, listen her events
	if ( con ) {
		con.on('data',function(data){ self.receiveChunk(data) });
		con.on('end', function(){ self.emit('end') });
		con.on('close', function(){ self.emit('close') });
		con.on('error', function(err){ self.emit('error',err) });
	}
...
// Methods
function msgStreamReceiveChunk(data) {
	...
	while ( offset < data.length ) {
		...
		if ( this.status == "header" ) {
			...
			if ( this.headerBufGot == 4 ) {
				...
				if ( this.msgBufTotal > this.MSG_MAXSIZE ) {
					console.log("[MessageStream] Ignoring message because if "+this.msgBufTotal+" bytes long (max is "+MSG_MAXSIZE+")");
