...
function Worker(opts) {
	...
	this._workerAbortWorks		= _workerAbortWorks;
...
//		_debug("> "+msg.toString('utf8'));
	...
	if ( m.command == "answer" ) {
		...
		if ( m.to == "offer" ) {
			...
			if ( m.error ) {
				_debug("[worker #"+self.id+"] Comrade manager didn't like my 'done': "+m.error.code+"("+m.error.description+")");
				return;
			}
	...
	// Abort
		...
		if ( m.work == null || !(m.work instanceof Array) ) {
			_debug("[worker #"+self.id+"] Comrade manager is sending me no work to abort, ignoring..");
			return;
		}

		return self._workerAbortWorks(m.work);
...
function _workerReceiveWork(works) {
	...
	acceptedWorks.forEach(function(id){
		...
		self.workingQueue[id].status = "running";
...
function _workerFinishWork(w,result) {
	...
	if ( w.status == "aborted" ) {
		_debug("[worker #"+self.id+"] Job "+w.id+" was previously aborted. Doing nothing.");
		return;
	}
...
function _workerAbortWorks(works) {

	var
		self = this;

	_debug("[worker #"+self.id+"] Aborting "+works.length+" works...");

	works.forEach(function(id){
		if ( typeof(id) != "string" || self.workingQueue[id] == null )
			return;

		self.workingQueue[id].status = "aborted";

		// Emit abort event

		self.emit('abort',self.workingQueue[id]);

		// Remove from the list

		self.workingQueue[id] = null;
//		delete self.workingQueue[id];

		// Now we can do something more

		self.availableSlots++;
	});

	_debug("[worker #"+self.id+"] I have now "+self.availableSlots+" available slots after aborting stuff");
...
function _workerOnDisconnect() {
	...
	for ( var id in self.sysMessages ) {
		self.sysMessages[id].handler(new Error("Disconnected from server before getting the confirmation"),null);
		delete self.sysMessages[id];
	}
