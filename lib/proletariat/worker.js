...
function workerStart() {
	...
	_debug("[worker #"+this.id+"] Starting (manager: "+this.host+")");
...
function _workerStartBiz() {
	...
	_debug("[worker #"+this.id+"] Offering "+this.availableSlots+" slots");
...
//		_debug("> "+msg.toString('utf8'));
	...
	if ( m.command == "answer" ) {
		...
		if ( m.to == "offer" ) {
			...
			if ( m.error ) {
				_debug("[worker #"+self.id+"] Comrade manager didn't accept my offer. I will try later.");
				return setTimeout(function(){
...
function _workerReceiveWork(works) {
	...
	if ( rejectedWorks.length > 0 ) {
		_debug("[worker #"+self.id+"] REJECTED "+rejectedWorks.length+" works");
...
function _workerFinishWork(w,result) {
	...
	_debug("[worker #"+self.id+"] Finished job "+w.id);
