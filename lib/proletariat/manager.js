...
var
	...
	MasterRepl			= require('./repl/master'),
...
function Manager(opts) {
	...
	// Validate options
	if ( !opts.port || (typeof opts.port != "number" && typeof opts.port != "string") || !parseInt(opts.port) )
		opts.port = 1917;
	if ( opts.master && typeof(opts.master) == "string" ) }
		if ( !opts.master.match(/:(\d+)$/) )
			self.masterAddr = opts.master.match(/:(\d+)$/) ? opts.master : opts.master+":1917";
	}
	if ( !opts.pingTimeout || (typeof opts.pingTimeout != "number" && typeof opts.pingTimeout != "string") || !parseInt(opts.pingTimeout) )
		opts.pingTimeout = 10000;
	if ( !opts.pingCheckInterval || (typeof opts.pingCheckInterval != "number" && typeof opts.pingCheckInterval != "string") || !parseInt(opts.pingCheckInterval) )
		opts.pingCheckInterval = 2000;
	...
	// Fixed properties
	self.socket				= null;
	self.bootTime			= (new Date()).getTime();
...
Manager.prototype.start = function() {
	...
	self.s = net.createServer(function(con){
		// Got a connection

		// Create the client connection instance
		client = new Client(con,idx++,self);

		// Register the client
		self.clientList.push(client);
		self.clients[client.id] = client;
	});
	self.s.listen(self.port, function(){
		_debug("Listening at "+self.port);

		// Connect to the master instance
		if ( self.masterAddr ) {
			self.master = new MasterRepl(self.masterAddr);
			self.master.start();
		}
	...
	// Watch ping times
	self._checkPingInterval = setInterval(function(){self.checkPing()},self.opts.pingCheckInterval);
...
Manager.prototype.stop = function() {
	...
	clearInterval(self._pingCheckInterval);

	// Stop server
	self.socket.close();
	self.socket = null;
	self._debug("остановившийся!");

}

// Check the ping time of every client
Manager.prototype.checkPing = function() {

	var
		self = this;

	self.clientList.forEach(function(client){
		client.checkPing();
	});
...
// Client object
function Client(con,idx,mgr) {

	var
		self = this;

	self._id		 = (new Date).getTime()+"."+idx;
	self.id			 = null;
	self.manager	 = mgr;
	self.opts		 = mgr.opts;
	self.connected	 = true;
	self.socket		 = con;
	self.pingTime	 = new Date();
	self.timeouts	 = [];
	...
	self.recvTimeout = self.setTimeout(function(){
		self._debug("Timeout while waiting for client hello. Disconnecting!");
		self.disconnect();
	},2000);
	self.send({c:"hello",proto_v:"1.0"});
...
Client.prototype.onError = function(err) {
	...
	self._debug("Client connection error: ",err);
...
Client.prototype.onDisconnect = function() {
	...
	self._debug("Client disconnected");
...
Client.prototype.onMessage = function(msg) {
	...
	try {
		...
		self._debug("Is comrade drunk or what? Got invalid JSON. Ignoring message: ",ex);
		self._debug("Original was: ",msg.toString('utf8'));
		return;
	}
	...
	switch ( m.c ) {
		...
		case "hello":
			if ( self.status !== "wait-hello" ) {
				self._debug("Client sent me and hello but he was not on a wait-hello status. Disconnecting...");
				return self.disconnect();
			}

			self._debug("Got hello from client"+(m.id?" ("+m.id+")":"")+". Client is ready!");
			self.status	= "ready";
			self.id = m.id;
		...
		default:
			self._debug("Got a weird message from client: ",m);
...
// Check the ping time
Client.prototype.checkPing = function(){

	var
		self	= this,
		now		= new Date();

	if ( self.pingTime <= now - self.opts.pingTimeout ) {
		self._debug("Ping timeout... Disconnecting!");
		return self.disconnect();
	}
...
Client.prototype._debug = function() {
	...
	_debug.apply(null,["[client "+this.id+"]"].concat(args));
...
Client.prototype.send = function(m) {
	...
	if ( !this.connected ) {
		this._debug("Trying to send messages to a disconnected client... discarding...");
		return;
	}
...
// Set a timeout
Client.prototype.setTimeout = function(callback,time) {
...
// Cancel all timeouts
Client.prototype.cancelTimeouts = function(){
...
function _clientInit(con) {
	...
	this.clients[con._id] = c = {
		...
		// Stream
		stream: new MessageStream(con,{format:"string"}),
...
//		process.nextTick(function(){ self.workDistribute(); });
	...
	// Abort (a request or a work) command
		...
		// Abort a request
		if ( m.rid ) {
			if ( !self._clientAbortRequest(c,c.id+" "+m.rid,"client request abort") )
		...
		// Abort specific works
			...
			var abortedIDs = self._clientAbortWork(c,m.work,"client work abort");
...
function _clientPushWork(c,rid,items,opts) {
	...
	items.forEach(function(item){
		...
		if ( self.HOLDSIMILARWORK && w.key != null && self.workKeys[w.key] ) {
		...
		if ( w.key )
			self.workKeys[w.key] = (self.workKeys[w.key] == null) ? 1 : self.workKeys[w.key]+1;
...
// Abort a request
function _clientAbortRequest(c,rid,reason) {
	...
	rv = self._clientAbortWork(c,works,reason);
...
function _clientAbortWork(c,works,reason) {
	...
	_debug("Aborting "+works.length+" work(s) from comrade "+c.id+" due to "+reason);
	...
	works.forEach(function(id){
		...
		_debug("Aborting client "+c.id+" work "+id+" ...");
		if ( w.status == "new" ) {
			// What can i do here ?
		}
		else if ( w.status == "onhold" ) {
			// Remove current job from holding list
...
//			delete worker.workRanges[id];
			...
			// There are other works waiting for me? Pick the next and submit him into the queue!
			if ( w.key && self.worksOnHold[w.key] && self.worksOnHold[w.key].length > 0 ) {
				var nextw = self.worksOnHold[w.key].shift();
				_debug("Pushing work "+nextw.id+" into the queue because his parent work was aborted");
				self._queuePush(nextw);
				nextw.status = "queued";
				distrib = true;
			}
...
// Cleanup
 ...
 function _cleanup() {
	...
	lists.forEach(function(list){
 	...
 	for ( var k in this.worksOnHold ) {
 		if ( this.worksOnHold[k].length == 0 )
 			delete this.worksOnHold[k];
 	}

 	for ( var k in this.workKeys ) {
 		if ( this.workKeys[k] == 0 )
 			delete this.workKeys[k];
 	}
