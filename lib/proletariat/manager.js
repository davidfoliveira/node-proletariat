...
//		_debug(c.id+" > ",JSON.stringify(m));
	...
	if ( m.command == "offer" ) {
		...
		_debug("Comrade "+c.id+" offered "+m.slots+" work slots. He will be considered a worker");
...
//		return process.nextTick(function(){ self._workDistribute(); });
		...
		c.type = "client";
		var ids = self._clientPushWork(c,m.work,{timeout: m.timeout});
...
function _clientRejectWork(c,works,m) {
	...
	if ( m )
		_debug("WARNING: Worker "+c.id+" REJECTED "+works.length+" works ("+JSON.stringify(works)+"). My idea was: "+parseInt(c.availableSlots+(m.accepted?m.accepted.length:0)+works.length)+", real: ",m.allocation);
...
//		_debug("WARNING: Global availability ("+self.globalAvailableSlots+") bellow threshold ("+AVAIL_THRESHOLD+"), wait...");
	...
	for ( var id in self.workers ) {
		if ( self.workers[id] )
			avHash[id] = self.workers[id].availableSlots;
	}
...
//		_debug("Distribute "+self.workQueue.length+", assignCount: "+assignCount+", gas: "+self.globalAvailableSlots);
		...
		for ( var cid in self.workers ) {
			...
			av = (workerAssigns[c.id] == null) ? c.availableSlots : c.availableSlots - workerAssignCount[c.id];
			if ( av == 0 )
				continue;

			count += av;
			if ( randFactor <= count ) {
				_debug("Work "+w.id+" assigned to worker "+c.id+" after "+_nsec(w.pushTime)+"ms");

				if ( workerAssigns[c.id] == null ) {
					workerAssigns[c.id] = [];
					workerAssignCount[c.id] = 0;
				}
				workerAssigns[c.id].push(w);
				workerAssignCount[c.id]++;
				assigned = c;
				assignCount++;
				break;
			}
	...
	_debug("Assigned "+assignCount+" works to "+Object.keys(workerAssigns).length+" workers. Dispatching...");

	// Dispatch works for each worker

	for ( var id in workerAssigns )
		self._workDispatchClient(self.workers[id],workerAssigns[id]);
