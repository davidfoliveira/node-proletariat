...
function Client(opts) {
	...
	this._clientRequestTerminate	= _clientRequestTerminate;
...
function _clientRequestWorkDone(req,works) {
	...
	self._clientRequestTerminate(req,results);

}

// Answer to the callback and terminate request

function _clientRequestTerminate(req,results) {
...
function clientWork(works,opts,handler) {
	...
	if ( opts.timeout ) {
		timeout = setTimeout(function(){
			...
			self._clientRequestTerminate(rid,errors);
			timedOut = true;
		},opts.timeout);
		delete opts['timeout'];
	}
...
function clientWorkIndividual(works,opts,iHandler,gHandler) {
	...
	if ( opts.timeout ) {
		timeout = setTimeout(function(){
			...
			// Has registered works?
			if ( self.requestWorks[rid] ) {
				requestWorks[rid].forEach(function(w){
					if ( self.worksWithoutAnswer[w.id] )
			...
			self._clientRequestTerminate(rid,results);
			timedOut = true;
		},opts.timeout);
		delete opts['timeout'];
	}
