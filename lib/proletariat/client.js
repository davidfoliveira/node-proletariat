...
function _clientConnect() {
	...
	self.s = net.connect({host: self.host, port: self.port}, function(){
		console.log("[client] Connected to comrade manager");
		self.connected = true;
...
//			console.log("err: ",err);
			if ( err.code == "ECONNREFUSED" ) {
				console.log("Could not connect to manager. Retrying in "+CON_RETRYTIME+"ms...");
			...
			console.log("[client] No error code, ignoring by logging: "+err.toString());
		}
	})
...
//		console.log("> "+msg.toString('utf8'));
		...
		console.log("[client] Is comrade manager drunk or what? Got invalid JSON. Ignoring message: ",ex);
		return;
	}
	...
	if ( m.command == "answer" ) {
		...
		if ( m.to == "push" ) {
			...
			if ( self.pendingPushAnswers.length == 0 ) {
				console.log("[client] Comerade manager is answering me to push but i didn't send any push, ignoring...");
				return;
			}
			...
			if ( m.error ) {
				console.log("[client #"+self.id+"] Comrade manager answered with error: ",m.error);
				return handler(err);
			}
...
//			console.log("[client] Comerade manager accepted my work! A glass of vodka for that!");

		}
		else {
			console.log("[client] Answer to something that I don't know");
		}
		return;
	}
...
//		console.log("Done");
		...
		for ( var req in reqs ) {
			if ( self.requestResults[req].length >= self.worksByRequest[req].length ) {
				console.log("Request #"+req+" reached the number of works: "+self.requestResults[req].length);
