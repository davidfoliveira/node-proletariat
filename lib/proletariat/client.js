...
function _clientOnMessage(msg) {
	...
	if ( m.command == "answer" ) {
		...
		if ( m.to == "push" ) {
			...
			self._clientRegisterWork(m.work,m.group,opts,handler);
	...
	// Work push ?
		...
		m.work.forEach(function(aw){
			...
			if ( self.requestOpts[w.request].individualHandler )
				self.requestOpts[w.request].individualHandler(aw.result.err,aw.result.data);
		});
...
function _clientRegisterWork(items,group,opts,handler) {
...
function clientWork(args,opts,handler) {
	...
	items = _clientWorkPrepare((args instanceof Array) ? args : [args]);
...
function clientWorkIndividual(args,opts,iHandler,gHandler) {
