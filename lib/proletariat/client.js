...
function _clientConnect() {
	...
	self.s.on('connect',function(){
		while ( self.waitingConnect.length > 0 ) {
			var
				handler = self.waitingConnect.shift().cb;
...
function _clientOnMessage(msg) {
	...
	if ( m.command == "answer" ) {
		...
		if ( m.to == "push" ) {
			...
			self.sysMessages[m.id].handler(null,m);
...
function _clientOnDisconnect() {
	...
	for ( var id in this.sysMessages ) {
		this.sysMessages[id].handler(new Error("Disconnected from server before getting the confirmation"),null);
		delete this.sysMessages[id];
	}
		...
		timeout = setTimeout(function(){
			...
			if ( iHandler ) {
				...
				if ( self.requestWorks[rid] ) {
					self.requestWorks[rid].forEach(function(wid){
						if ( self.worksWithoutAnswer[wid] ) {
							if ( iHandler )
								iHandler({code:"ECSTO"},null,self.works[wid]);
						}
					});
				}
				// No registered works? Just a bunch of errors..
				else {
					self.requestItems[rid].forEach(function(item){
						if ( iHandler )
							iHandler({code:"ECSTO"},null,{original: item});
					});
				}
			}
