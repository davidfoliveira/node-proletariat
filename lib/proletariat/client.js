...
function Client(opts) {
	...
	this._clientWorksTimeout	= _clientWorksTimeout;
...
function clientWork(works,opts,handler) {
	...
	if ( opts.timeout ) {
		setTimeout(function(){
			if ( self.requestItems[rid] )
				self._clientWorksTimeout(rid,works);
		}, opts.timeout);
	}
	if ( opts.runTimeout )
...
// Fire timeout on the works

function _clientWorksTimeout(rid,works,handler) {

	var
		self = this,
		handlers = self.requestHandlers[rid],
		err = {code: "ECSTO", message: 'Client-side timeout'};

	_debug("[client] Request "+rid+" works timed-out");

	if ( !handler )
		handler = function(){};

	// Works already registered
	if ( self.requestWorks[rid] ) {
		self.requestWorks[rid].forEach(function(id){
			var
				w = self.works[id||''];

			if ( w && self.worksWithoutAnswer[id] ) {
				self.requestResults[rid].push({id: id, result: {err:err, data: null}});
				if ( handlers && handlers.individual )
					handlers.individual(err,null,w);
			}
		});
	}

	// Works not registered
	else {
		works.forEach(function(w){
			self.requestResults[rid].push({id: w.id, result: {err:err, data: null}});
			if ( handlers && handlers.individual )
				handlers.individual(err,null,w);
		});
	}

	return self._clientRequestWorkDone(rid,self.requestResults[rid]);
